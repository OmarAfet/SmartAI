const Input = ({ name, classes, value, onBlur, onChange, type, placeholder, onKeyDown }) => {
	return (
		<>
			...
			<label htmlFor={placeholder} className={`absolute right-0 mx-[10px] px-1 text-gray-400 cursor-text inline-block top-2 peer-focus:-top-2.5 peer-focus:text-xs duration-150 ${value ? "text-xs -top-2.5" : ""} bg-white`}>{placeholder}</label>
		</>
	);
};
